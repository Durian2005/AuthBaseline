...
均在 `tools/evidence/`，但**未随仓库分发**：

> 该目录被 `.gitignore` 整目录排除，原因不是"体积"，而是**内容**：
> 界面截图里显示着真实账号名（审计页与用户管理页的账号列），
> 这类信息没法像文本那样扫出来脱敏，唯一的办法就是不传；
> 快照文件 `chain_backup_before_repair.json` 则是 322 条真实操作日志的导出。
> 因此它们只留在本机，不属于开源分发的范围 —— 需要复现时，
> 按本文档各节的命令在本地重跑一遍即可（脚本在 `tools/` 下，均已入库）。
> 下面清单的作用因此是"证据索引"，说明每一项该长什么样、该看哪张图。
