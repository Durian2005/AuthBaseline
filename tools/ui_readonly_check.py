# -*- coding: utf-8 -*-
"""启动桌面端（带 WebView2 调试端口）→ 验证审计页无任何"删日志"入口 → 收尾。
...
用法：
  python tools/ui_readonly_check.py
环境变量：
  E2E_APPDIR  安装目录，默认 %LOCALAPPDATA%\\AuthBaseline
  E2E_USER / E2E_PASS  登录凭据（凭据不硬编码，换库时不会静默失败）
"""
import os
import subprocess
import sys
...
def main():
    ...
    log = ROOT / "tools" / "buildlogs" / "ui_readonly_app.log"
    ...
    r = subprocess.run(
        [NODE, str(ROOT / "tools" / "ui_readonly_events.mjs")],
